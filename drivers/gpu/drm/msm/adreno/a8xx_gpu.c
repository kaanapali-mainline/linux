...
static void a8xx_nonctxt_config(struct msm_gpu *gpu)
{
	struct adreno_gpu *adreno_gpu = to_adreno_gpu(gpu);
...
static int hw_init(struct msm_gpu *gpu)
{
	...
	a8xx_write_pipe(gpu, PIPE_BR, REG_A8XX_RB_GC_GMEM_PROTECT, 0x12000000);
	/* Clear aperture */
	a8xx_aperture_set(gpu, 0);
...
static void a8xx_cp_hw_err_irq(struct msm_gpu *gpu)
{
	...
	if (sw_fault_mask & status) {
		...
		if (status & BIT(8)) {
			...
			status = a8xx_read_pipe_slice(gpu, pipe, slice, REG_A8XX_CP_SQE_STAT_DATA_PIPE);
			dev_err_ratelimited(&gpu->pdev->dev,
